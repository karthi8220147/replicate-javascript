 ...
 *
 * @param {string} logs - Logs from the prediction.
 * @returns {object} - An object with the percentage, current and total.
 */
function parsePredictionProgress(logs) {
  if (!logs) {
    return null;
  }
  ...
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    ...
    if (matches && matches.length === 4) {
      const percentage = parseInt(matches[1], 10) / 100;
...
module.exports = { withAutomaticRetries, parsePredictionProgress };
